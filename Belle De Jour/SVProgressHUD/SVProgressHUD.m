...
+ (void)show {
	[SVProgressHUD sharedView].displayRect = CGRectZero;
    [[SVProgressHUD sharedView] showWithStatus:nil maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
}

+ (void)showInRect:(CGRect)displayRect{
	[SVProgressHUD sharedView].displayRect = displayRect;
	[[SVProgressHUD sharedView] showWithStatus:nil maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
}
...
- (void)positionHUD:(NSNotification*)notification {
	...
	if (!CGRectIsEmpty([SVProgressHUD sharedView].displayRect)) {
		CGRect rect = [SVProgressHUD sharedView].displayRect;
		posX = rect.origin.x + rect.size.width/2.0;
		posY = rect.origin.y + rect.size.height/2.0;
	}
    ...
    switch (orientation) { 
        case UIInterfaceOrientationPortraitUpsideDown:
            rotateAngle = M_PI; 
            newCenter = CGPointMake(orientationFrame.size.width-posX, orientationFrame.size.height-posY);
            break;
        case UIInterfaceOrientationLandscapeLeft:
            rotateAngle = -M_PI/2.0f;
            newCenter = CGPointMake(posY, orientationFrame.size.width-posX);
            break;
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        ...
        stringLabel.font = [UIFont fontWithName:@"Futura" size:16.0];
